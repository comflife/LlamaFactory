...
Command to run ALL listed models (as openrouter:...) + local Qwen2-VL-2B-Instruct (common HF ID; adjust if different):

```
python scripts/orad3d_vlm_trajectory_fewshot_v2.py \
  --model-spec openrouter:qwen/qwen3-vl-8b-instruct \
  --model-spec openrouter:qwen/qwen3-vl-32b-instruct \
  --model-spec openrouter:openai/gpt-5.2 \
  --model-spec openrouter:openai/gpt-4o \
  --model-spec openrouter:google/gemini-3-flash-preview \
  --model-spec openrouter:google/gemini-3-pro-preview \
  --model-spec openrouter:anthropic/claude-opus-4.5 \
  --model-spec openrouter:anthropic/claude-sonnet-4.5 \
  --model-spec openrouter:anthropic/claude-haiku-4.5 \
  --model-spec openrouter:meta-llama/llama-4-maverick \
  --model-spec openrouter:meta-llama/llama-4-scout \
  --model-spec local:Qwen/Qwen2-VL-2B-Instruct \
  --openrouter-api-key $OPENROUTER_API_KEY \
  --orad-root /home/work/datasets/bg/ORAD-3D \
  --split testing \
  --out-dir /home/work/byounggun/LlamaFactory/orad3d_all_models

python scripts/orad3d_vlm_trajectory_fewshot_v3.py \
  --model-spec openrouter:qwen/qwen3-vl-8b-instruct \
  --model-spec openrouter:qwen/qwen3-vl-32b-instruct \
  --model-spec openrouter:openai/gpt-4o \
  --model-spec openrouter:google/gemini-3-flash-preview \
...
python scripts/orad3d_vlm_trajectory_fewshot_v3.py \
  --model-spec local:Qwen/Qwen3-VL-2B-Instruct \
  --openrouter-api-key "$OPENROUTER_API_KEY" \
  --orad-root /home/work/datasets/bg/ORAD-3D \
  --split testing \
  --out-dir /home/work/byounggun/LlamaFactory/orad3d_all_models \
...
- Set OPENROUTER_API_KEY env or --openrouter-api-key.
- Processes ALL testing samples.
- Outputs per-model manifest.jsonl with GT + preds.
- Local model downloads to --cache-dir (default ~/.cache/hf).
...
def _run_spec(
    ...
    model_dir = args.out_dir / spec.name
    model_dir.mkdir(exist_ok=True, parents=True)
    manifest_path = model_dir / "manifest.jsonl"
    done_keys = _load_manifest_keys(manifest_path)
    if done_keys:
        print(f"[RESUME {spec.name}] {len(done_keys)} samples already in {manifest_path}")

    written = 0
    skipped = 0
    with open(manifest_path, "a", encoding="utf-8") as f:
        for sample in test_samples:
            if sample.key in done_keys:
                skipped += 1
                continue
            out_text = ""
            ...
            if spec.model_type == "local":
                ...
                try:
                    ...
                    print(f"[ERROR {spec.name}] {sample.key}: {exc}")
                    continue

            traj_pts = _extract_traj_xyz(out_text)
            valid = len(traj_pts) >= args.min_points
            mean_l2 = _mean_l2(sample.gt_points, traj_pts)
            final_l2 = _final_l2(sample.gt_points, traj_pts)
            result = {
                "key": sample.key,
    ...
    return spec.name, written, skipped, manifest_path
...
def parse_args():
    ...
    ap.add_argument(
        ...
        nargs="?",
        default=None,
        help="If omitted, uses OPENROUTER_API_KEY from env/.env",
    )
    ...
    ap.add_argument(
        ...
        type=int,
        default=1,
        help="Parallelize openrouter models (threads). Local models remain sequential.",
    )
...
def main():
    ...
    print(f"[SAMPLES] {len(test_samples)} test samples (all available)")

    openrouter_specs = [s for s in specs if s.model_type == "openrouter"]
    local_specs = [s for s in specs if s.model_type == "local"]

    def _print_done(name: str, written: int, skipped: int, manifest_path: Path) -> None:
        print(f"[DONE {name}] {written} new, {skipped} skipped -> {manifest_path}")

    if args.parallel_models > 1 and len(openrouter_specs) > 1:
        print(f"[PARALLEL] OpenRouter models: {len(openrouter_specs)} with {args.parallel_models} threads")
        with ThreadPoolExecutor(max_workers=args.parallel_models) as executor:
            futures = [
                executor.submit(_run_spec, spec, args, train_examples, test_samples)
                for spec in openrouter_specs
            ]
            for future in as_completed(futures):
                name, written, skipped, manifest_path = future.result()
                _print_done(name, written, skipped, manifest_path)
    else:
        for spec in openrouter_specs:
            name, written, skipped, manifest_path = _run_spec(spec, args, train_examples, test_samples)
            _print_done(name, written, skipped, manifest_path)
