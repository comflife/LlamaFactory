...
Example (3 adapters):
python scripts/occupancy_gt_with_eval_saved.py \
...
Example (adapter + saved API manifest):
python scripts/occupancy_gt_with_eval_saved.py \
  --dataset-root /home/work/datasets/bg/ORAD-3D --split testing --scene y0602_1309 \
  --mode topdown --save-dir occupancy_plots_multi \
  --path-source local_path_ins --path-frame local \
  ...
  --base-model Qwen/Qwen3-VL-2B-Instruct \
  --adapter sft=/home/work/datasets/bg/byounggun/saves/orad3d/qwen3-vl-2b/lora/sft_v2_8/checkpoint-1044 \
  --saved-manifest gpt-4o=/home/work/byounggun/LlamaFactory/orad3d_all_models/gpt-4o \
  --image-folder image_data \
  --temperature 1e-6
...
def _resolve_manifest_path(path: str) -> str:
    if os.path.isdir(path):
        cand = os.path.join(path, "manifest.jsonl")
        if os.path.isfile(cand):
            return cand
    return path

def _parse_saved_manifest_specs(values: Sequence[str]) -> List[SavedManifestSpec]:
    if not values:
        return []
    specs: List[SavedManifestSpec] = []
    seen: set[str] = set()
    for idx, raw in enumerate(values):
        name = raw
        path = raw
        ...
        path = (path or "").strip()
        if not path:
            raise SystemExit(f"Invalid saved-manifest spec: {raw}")
        path = _resolve_manifest_path(path)
        if not os.path.isfile(path):
            raise SystemExit(f"Saved manifest not found: {path}")
        if not name:
            base = Path(path).parent.name if Path(path).name == "manifest.jsonl" else Path(path).stem
            name = base or f"saved{idx + 1}"
        base_name = name
        suffix = 2
        while name in seen:
            name = f"{base_name}_{suffix}"
        ...
        seen.add(name)
        color = MODEL_COLORS[len(specs) % len(MODEL_COLORS)]
        specs.append(SavedManifestSpec(name=name, path=path, color=color))
    return specs

def _load_manifest_index(path: str) -> Dict[str, Dict[str, Any]]:
...
def _normalize_ts_candidates(ts: str) -> List[str]:
    ts = (ts or "").strip()
    if not ts:
        return []
    candidates = {ts}
    try:
        candidates.add(str(int(float(ts))))
    except Exception:
        pass
    return list(candidates)

def _lookup_manifest_entry(
    index: Dict[str, Dict[str, Any]],
    split: str,
    scene: str,
    occ_ts: str,
) -> Optional[Dict[str, Any]]:
    for ts in _normalize_ts_candidates(occ_ts):
        key = f"{split}_{scene}_{ts}"
        if key in index:
            return index[key]
        key2 = f"{scene}_{ts}"
        if key2 in index:
...
def _extract_saved_points(entry: Optional[Dict[str, Any]]) -> Optional[np.ndarray]:
    if not entry:
        return None
    if entry.get("valid") is False:
        return None
    for key in ("trajectory_points", "pred_points", "points", "path"):
        pts = entry.get(key)
...
def _count_violations_on_labels(
    *,
    ...
    if path_xyz is None or label_grid is None or axes not in ("xy", "yx"):
        return None
    path_xy = _path_to_axes(path_xyz, axes, flip_x=flip_x, flip_y=flip_y)
    if path_xy.size == 0:
        return (0, 0)
    ...
    return (int(hit.sum()), int(valid.sum()))
...
def _plot_topdown_multi(
    *,
    ...
    # --------- paths ----------
    # GT
    gt_xy = _path_to_axes(gt_path, axes, flip_x=flip_x, flip_y=flip_y)
    if gt_xy.size:
    ...
    for name, mp, color in model_paths:
        if mp is None or mp.size == 0:
            continue
        mxy = _path_to_axes(mp, axes, flip_x=flip_x, flip_y=flip_y)
        if mxy.size < 2:
            continue
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="append",
        default=None,
        help="Saved manifest.jsonl path or name=path (repeatable). Directory is allowed (manifest.jsonl auto).",
    )
...
def main() -> int:
    ...
    saved_specs = _parse_saved_manifest_specs(args.saved_manifest or [])
    saved_indexes = {spec.name: _load_manifest_index(spec.path) for spec in saved_specs}
    ...
    for scene in sample_scenes:
        ...
        for occ_path in occ_list:
            ...
            # -------- Saved manifest paths (API) ----------
            if saved_specs:
                for spec in saved_specs:
                    entry = _lookup_manifest_entry(saved_indexes.get(spec.name, {}), args.split, scene, occ_ts)
                    pts = _extract_saved_points(entry)
                    if pts is None:
                        model_paths_local.append((spec.name, None, spec.color))
                        continue
                    ...
                    model_paths_local.append((spec.name, pts, spec.color))
            ...
            label_grid_for_path = None
            if points_are_grid and occ_points is not None and occ_labels is not None:
                _, label_grid_for_path = _grid_from_occ_points(occ_points, occ_labels, args.label_proj)
            ...
            if label_grid_for_path is not None and args.topdown_axes in ("xy", "yx"):
                parts = [f"{scene} {occ_ts}"]
                gt_stats = _count_violations_on_labels(
                    path_xyz=gt_path,
                    label_grid=label_grid_for_path,
                ...
                if gt_stats is None:
                    ...
                    hit, total = gt_stats
                    status = "HIT" if hit > 0 else "OK"
                    parts.append(f"GT {hit}/{total} {status}")

                for name, mp, _ in model_paths_local:
                    if mp is None or mp.size == 0:
                        parts.append(f"{name} no-path")
                        continue
                    stats = _count_violations_on_labels(
                        path_xyz=mp,
                        label_grid=label_grid_for_path,
                        axes=args.topdown_axes,
                        flip_x=args.flip_x,
                    ...
                    if stats is None:
                        ...
                        hit, total = stats
                        status = "HIT" if hit > 0 else "OK"
                        parts.append(f"{name} {hit}/{total} {status}")

                print("[VIOL]", " | ".join(parts))
