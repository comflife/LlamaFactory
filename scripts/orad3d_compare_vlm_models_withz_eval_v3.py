...
Example:
python scripts/orad3d_compare_vlm_models_withz_eval_v3.py \
  --base-model Qwen/Qwen3-VL-2B-Instruct \
  --adapter sft_refine=/home/work/datasets/bg/byounggun/saves/orad3d/qwen3-vl-2b/lora/sft_v2_refine/checkpoint-5220 \
  --adapter sft=/home/work/datasets/bg/byounggun/saves/orad3d/qwen3-vl-2b/lora/sft_v2_8/checkpoint-5220 \
  ...
  --hit-threshold 2.0 --traj-step-sec 0.5 --traj-start-sec 0.0
"""
...
def _parse_horizon_seconds(raw: str) -> List[Tuple[str, float]]:
    ...
    if not values:
        return []
    return [(f"{v:g}s", v) for v in values]


def _interp_points(a: Sequence[float], b: Sequence[float], alpha: float) -> List[float]:
    alpha = max(0.0, min(1.0, float(alpha)))

    def get(p: Sequence[float], k: int) -> float:
        return float(p[k]) if len(p) > k else 0.0

    return [
        (1 - alpha) * get(a, 0) + alpha * get(b, 0),
        (1 - alpha) * get(a, 1) + alpha * get(b, 1),
        (1 - alpha) * get(a, 2) + alpha * get(b, 2),
    ]


def _point_xyz(p: Sequence[float]) -> List[float]:
    return [
        float(p[0]),
        float(p[1]),
        float(p[2]) if len(p) > 2 else 0.0,
...
def _interp_at_time(
    ...
    if not points or step_sec <= 0:
        return None
    end_time = start_sec + step_sec * (len(points) - 1)
    if t_sec < start_sec or t_sec > end_time:
        return None
    if len(points) == 1:
        return _point_xyz(points[0])
    if t_sec >= end_time:
        return _point_xyz(points[-1])

    t_rel = t_sec - start_sec
    i0 = int(math.floor(t_rel / step_sec))
    if i0 < 0:
        i0 = 0
    if i0 >= len(points) - 1:
        return _point_xyz(points[-1])
    i1 = i0 + 1
    alpha = (t_rel - i0 * step_sec) / step_sec
    return _interp_points(points[i0], points[i1], alpha)


def _resample_points_at_gt_times(
    points: List[List[float]],
    gt_len: int,
    start_sec: float,
    step_sec: float,
) -> List[List[float]]:
    if gt_len <= 0 or not points:
        return []
    if step_sec <= 0:
        return _resample_points(points, gt_len)
    out: List[List[float]] = []
    for i in range(gt_len):
        t_sec = start_sec + i * step_sec
        p = _interp_at_time(points, t_sec, start_sec, step_sec)
        if p is None:
            continue
        out.append(p)
    return out
...
def _compute_adapter_metrics(
    *,
    ...
    hit_threshold: float,
    metric_horizons: Sequence[Tuple[str, float]],
    traj_step_sec: float,
    traj_start_sec: float,
    ...
    horizon_labels = [label for label, _ in metric_horizons]
    horizon_count = len(metric_horizons)
    ...
    for adapter in adapters:
        sum_l2 = [0.0 for _ in range(horizon_count)]
        count_l2 = [0 for _ in range(horizon_count)]
        fail_count = [0 for _ in range(horizon_count)]
        ...
        for item in items:
            ...
            horizon_l2: List[Optional[float]] = [None for _ in range(horizon_count)]
            horizon_fail_flags: List[Optional[bool]] = [None for _ in range(horizon_count)]
            if horizon_count and traj_step_sec > 0:
                for h_idx, (_, h_sec) in enumerate(metric_horizons):
                    gt_p = _interp_at_time(gt, h_sec, traj_start_sec, traj_step_sec)
                    pred_p = _interp_at_time(pred, h_sec, traj_start_sec, traj_step_sec)
                    if gt_p is None or pred_p is None:
                        continue
                    d_xy = _l2_point_xy(gt_p, pred_p)
                    horizon_l2[h_idx] = d_xy
                    sum_l2[h_idx] += d_xy
                    count_l2[h_idx] += 1
                    fail = d_xy > hit_threshold
                    horizon_fail_flags[h_idx] = fail
                    if fail:
                        fail_count[h_idx] += 1
            ...
            if collect_samples:
                ...
                for h_idx, label in enumerate(horizon_labels):
                    row[f"l2@{label}"] = horizon_l2[h_idx]
                    row[f"fail@{label}"] = horizon_fail_flags[h_idx]
                rows.append(row)

        metrics_entry: Dict[str, Any] = {
            "adapter": adapter.name,
        }
        mean_l2s: List[Optional[float]] = []
        total_fail = 0
        total_count = 0
        for h_idx, label in enumerate(horizon_labels):
            mean_l2 = (sum_l2[h_idx] / count_l2[h_idx]) if count_l2[h_idx] else None
            mean_l2s.append(mean_l2)
            metrics_entry[f"L2@{label}"] = mean_l2
            total_fail += fail_count[h_idx]
            total_count += count_l2[h_idx]
        if horizon_count:
            metrics_entry["L2_avg"] = _mean_or_none(mean_l2s)
            metrics_entry["failure_rate_avg"] = (total_fail / total_count) if total_count else None
...
def _render_model_panel(
    *,
    ...
    for output, color in model_outputs:
        ...
        if gt_points:
            plot_pts = _resample_points_at_gt_times(pts, len(gt_points), traj_start_sec, traj_step_sec)
        else:
            plot_pts = pts
...
def _make_composite(
    *,
    ...
    panel = _render_model_panel(
        ...
        forward_axis=forward_axis,
        flip_lateral=flip_lateral,
        traj_start_sec=traj_start_sec,
        traj_step_sec=traj_step_sec,
    )
    ...
    model_points = [
        (
            out.name,
            _resample_points_at_gt_times(out.trajectory_points, len(gt_points), traj_start_sec, traj_step_sec),
            color,
        )
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--debug-metrics", action="store_true", help="Save per-sample horizon L2/failure stats for debugging.")
    ...
    ap.add_argument(
        ...
        type=float,
        default=0.1,
        help="Seconds between consecutive GT points (used for horizon L2).",
    )
    ap.add_argument(
        "--traj-start-sec",
        type=float,
        default=0.0,
        help="Timestamp of the first GT point (set to traj-step-sec if GT starts at 1 step).",
    )
    ap.add_argument(
        "--metric-horizons",
        type=str,
        default="1,2,3",
        help="Comma-separated horizon seconds for L2 metrics (e.g., 1,2,3).",
    )
    ...
    ap.add_argument(
        ...
        type=float,
        default=2.0,
        help="Failure threshold in meters (L2@horizon in XY).",
    )
...
def main() -> int:
    ...
    with manifest_path.open("w", encoding="utf-8") as f:
        for item in items:
            ...
            comp = _make_composite(
                ...
                traj_start_sec=float(args.traj_start_sec),
    ...
    metric_horizons = _parse_horizon_seconds(args.metric_horizons)
    ...
    if metrics:
        ...
        for entry in metrics:
            ...
            if metric_horizons:
                for label, _ in metric_horizons:
                    key = f"L2@{label}"
                    parts.append(
                        f"{key}={entry[key]:.3f}" if entry.get(key) is not None else f"{key}=n/a"
                    )
                parts.append(
                    f"L2_avg={entry['L2_avg']:.3f}" if entry.get("L2_avg") is not None else "L2_avg=n/a"
                )
                parts.append(
                    f"failure_rate_avg={entry['failure_rate_avg']:.3f}"
                    if entry.get("failure_rate_avg") is not None else "failure_rate_avg=n/a"
                )
            print("[METRICS] " + " ".join(parts))

    if bool(args.debug_metrics) and sample_rows:
        for name, rows in sample_rows.items():
            out_path = args.out_dir / f"sample_metrics_{name}.jsonl"
