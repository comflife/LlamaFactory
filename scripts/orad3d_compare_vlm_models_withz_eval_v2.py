...
Example:
python scripts/orad3d_compare_vlm_models_withz_eval_v2.py \
    ...
    for adapter in adapters:
        ...
        for item in items:
            ...
            for i in range(gt_len):
                ...
                pred_p = _interp_at_frac(pred, frac)
                d_xy = _l2_point_xy(gt[i], pred_p)
                d_xyz = _l2_point_xyz(gt[i], pred_p)
                total_xy += d_xy
                total_xyz += d_xyz
                if d_xy > max_xy:
                    max_xy = d_xy
            ...
            pred_last = _interp_at_frac(pred, 1.0)
...
def _render_model_panel(
    *,
    ...
    for output, color in model_outputs:
        ...
        plot_pts = _resample_points(pts, len(gt_points)) if gt_points else pts
...
def _make_composite(
    *,
    ...
    model_points = [
        (out.name, _resample_points(out.trajectory_points, len(gt_points)), color)
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--debug-metrics", action="store_true", help="Save per-sample ADE/FDE/HitRate stats for debugging.")
